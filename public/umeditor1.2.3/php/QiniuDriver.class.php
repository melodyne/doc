...
class QiniuDriver{
	 ...
	 * @param    array  $this->config  ueditor 配置信息
	 * @return   array
	 * @author   widuu <admin@widuu.com>
	 */

	public function fetchFile( $image_url){
		// 要抓取的URL
		...
		// 文件名称
		$file_name   = $this->getFileName($field_name,$this->config,true); 
	 ...
	 * 设置上传信息
	 *
	 * @param 	array  $this->config ueditor json配置信息
	 * @return 	array  
	 * @author  widuu <admin@widuu.com>
	 */

	public function getFileInfo(){
		$file_info  = $this->setUploadInfo();
	...
	private function setUploadInfo(){
		$field_name  = $this->config['fieldName'];
		$file_name   = $this->getFileName($field_name);
		$upload_info = array(
	        "name"		=> 'file',
	        'file_name'	=> $file_name,
	...
	private function getFileName( $field_name , $flag = false ){
		$prefix = trim($this->config['qiniuUploadPath'] , "/" ) . "/" ;
		$format = $this->config['qiniuDatePath'];
